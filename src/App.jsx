...
export default function App() {
  ...
  const DEFAULT_TAB_ORDER = ["roster","scan","overview","program","alumni"];
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        if (metaData?.data) {
          setSeason(metaData.data.season || 1);
          setTeamName(metaData.data.teamName || "My Dynasty");
          if (metaData.data.tabOrder && Array.isArray(metaData.data.tabOrder)) {
            // merge with defaults in case new tabs were added since saving
            const saved = metaData.data.tabOrder.filter(t => DEFAULT_TAB_ORDER.includes(t));
            const missing = DEFAULT_TAB_ORDER.filter(t => !saved.includes(t));
  ...
  const saveToSupabase = useCallback(async (newRoster, newAlumni, newProgram, newSeason, newTeamName, newTabOrder) => {
    if (!user) return;
    setSyncing(true);
    setLastSaved(null);
    try {
      ...
      const { error: me } = await supabase.from("meta").upsert({ user_id: user.id, data: { season: newSeason, teamName: newTeamName, tabOrder: newTabOrder || tabOrder }, updated_at: new Date().toISOString() });
  ...
  const handleTeamNameChange = useCallback((name) => {
    setTeamName(name);
    saveToSupabase(roster, alumni, program, season, name, tabOrder);
  }, [roster, alumni, program, season, tabOrder, saveToSupabase]);
  ...
  const reorderTabs = useCallback((fromKey, toKey) => {
    setTabOrder(prev => {
      ...
      saveToSupabase(roster, alumni, program, season, teamName, next);
      return next;
    });
  ...
  const TAB_LABELS = { roster:"Roster", scan:"📸 Scan", overview:"Overview", program:"◆ Program", alumni:"Alumni" };
  const orderedTabs = tabOrder.map(key => [key, TAB_LABELS[key]]).filter(([,lbl])=>lbl);
        ...
        <div style={{ display:"flex", gap:6, flexWrap:"wrap", alignItems:"center" }}>
          {orderedTabs.map(([t,lbl])=>(
            <button
              ...
              onDrop={e=>{ e.preventDefault(); if(draggingTab && draggingTab!==t) reorderTabs(draggingTab, t); setDraggingTab(null); }}
              onDragEnd={()=>setDraggingTab(null)}
              style={{ ...S.tabBtn(tab===t), position:"relative", opacity:draggingTab===t?0.4:1, cursor:"grab" }}
